...
def main_get():
    return render_template('index.html', not_valid=False, developer_name='Huseyin')
...
def main_post():
    ...
    if not (0 < number):
        return render_template('index.html', not_valid=True, developer_name='Huseyin')
    return render_template('result.html', milliseconds=number, result=convert(number), developer_name='Huseyin')
